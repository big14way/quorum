...
the Squads v4 program source. Discriminators, base58 constants, and PDA
vectors in the test suite were generated by an independent pure Python
implementation (sha256 plus a from scratch ed25519 decompression check), so
the Rust code is verified against something it did not produce. A mainnet
fixture pass is scheduled before submission to pin the encoders against a
real proposal created by the official Squads app.
